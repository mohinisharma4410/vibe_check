...
export default function VoiceScreen({ update, nextStep }) {
  const [phase,      setPhase]      = useState('idle') // idle | recording | done | unsupported
  ...
  // Fix: track phase in a ref so callbacks don't close over stale value
  ...
  function startRecording() {
    ...
    // Fix: use phaseRef instead of stale phase from closure
    rec.onend = () => {
  ...
  const dashoffset = circ * (1 - Math.min(seconds / 30, 1))
      ...
      {phase === 'unsupported' ? (
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: 140, height: 140, position: 'relative', marginBottom: 20 }}>
            ...
            <button
              ...
              style={{
                ...
                fontSize: 26,
                background: phase === 'recording' ? '#d64c3b' : phase === 'done' ? 'var(--mocha)' : 'var(--brown-dark)',
                color: 'var(--latte)',
          ...
          <div style={{ textAlign: 'center', marginBottom: 16 }}>
            {phase === 'idle'      && <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>Tap to start recording</p>}
            {phase === 'recording' && (
              <div>
                <p style={{ color: '#d64c3b', fontFamily: 'var(--font-display)', fontWeight: 600 }}>
                  Recording… {seconds}s / 30s
                </p>
                <p style={{ color: 'var(--text-muted)', fontSize: 12, marginTop: 4 }}>Tap the square to stop</p>
              </div>
            )}
            {phase === 'done' && (
          ...
          {transcript && (
            <div style={{ background: 'var(--cream2)', borderRadius: 14, padding: '14px 16px', width: '100%', animation: 'fadeUp 0.3s both' }}>
              <div style={{ fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6, fontWeight: 500 }}>
                What we heard
              </div>
